...
- *Calendar tokens* (~today~, ~yesterday~, ~tomorrow~, ~last friday~,
  ~this week~, ~this month~, ~this year~, …) — boundaries from the
  ~two-timer~ crate. ~today~ in a Wednesday reference =
  ~[Wed 00:00, Thu 00:00)~.

- *Explicit / terse formats* — ~L~ is the parsed instant with
  unspecified fields filled to start-of-unit; ~R~ is ~L~ plus *one
  unit of the smallest explicitly-specified field*. The unit comes
  from the rightmost ~%X~ specifier of the matched format:

  | Smallest specifier | Unit added to ~L~ to get ~R~ |
  |--------------------+------------------------------|
  | ~%Y~               | 1 calendar year              |
  | ~%m~               | 1 calendar month             |
  | ~%d~               | 1 day                        |
  ...
  So ~9h~ as a timespan is ~[09:00, 10:00)~ (1 hour); ~14:30~ is
  ~[14:30, 14:31)~ (1 minute); ~2024~ is the whole year; ~2024-10~
  is the whole month.

- *~now~* (case-insensitive) — the reference instant, treated as a
  zero-width period. Recognized only as a ~..~ operand or as the
  implicit right side of ~X..~.
